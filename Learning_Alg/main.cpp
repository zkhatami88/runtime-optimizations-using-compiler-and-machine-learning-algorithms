...
int main(int argc, const char * argv[]) {
    ...
    //MatrixXf experimental_results_two_class = MatrixXf::Random(number_of_experiments_two_class, number_of_features_two_class);
    targets = new int[number_of_experiments_two_class];
    std::ifstream myfile ("training_data_two_class.txt");

    std::size_t e = 0;
    while(e < number_of_experiments_two_class) {
        getline(myfile, line);
        std::stringstream ss(line);
        std::string str;
        std::size_t f = 0;
        while(getline(ss, str, '\t')) {
            if(f < number_of_features_two_class) {
                experimental_results_two_class[e][f] = std::atof(str.c_str());
            }
            else {
	...
	learning_binary_regression_model my_nw(number_of_experiments_two_class, number_of_features_two_class,
						                  threshold, experimental_results_two_class, targets);
