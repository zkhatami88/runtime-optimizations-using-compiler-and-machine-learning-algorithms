...
int main(int argc, const char * argv[]) {
	...
	experimental_results = new float*[number_of_experiments];
	targets = new int[number_of_experiments];

	for(std::size_t i = 0; i < number_of_experiments; i++) {
    	experimental_results[i] = new float[number_of_features];
    }
    ...
    */
	...
	learning_binary_regression_model my_nw(number_of_experiments, number_of_features, 
						      number_of_two_classes, threshold, experimental_results, targets);
    ...
    float** weightsm = my_nw.retrieving_weights_multi_classes();
